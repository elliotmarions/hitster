-- =====================================================================
--  LÅTSNURRAN – Bara original (hoppa över låtar som bara finns som remix)
--
--  0033 rankade original först men spelade ändå en remix om det var enda
--  previewbara träffen. Nu HÅRDFILTRERAS: bara en artist-matchande ORIGINAL-
--  träff accepteras. Hittas ingen → svaret behandlas som "ingen träff", så
--  servern breddar sökningen och därefter väljer en HELT ANNAN låt. En låt vars
--  enda träff är remix/karaoke/live/cover ... spelas alltså inte alls.
--
--  Original = previewbar + artistnamnet matchar + albumet är inte karaoke/
--  tribute/8-bit o.dyl. + trackName ANTINGEN exakt lika med titeln (så låtar som
--  faktiskt HETER något med "live"/"cover" i titeln alltid släpps igenom) ELLER
--  saknar remix/karaoke/live ...-markörer.
--
--  Medveten avvägning ("bara original just nu"): saknas ett spelbart original
--  helt, hoppas låten över – hellre det än en remix. Lätt att mjuka upp igen
--  senare (återgå till 0033:s ORDER BY-ranking). Additiv + idempotent.
--  Kör efter 0033.
-- =====================================================================
...
begin
  ...
  if coalesce(v_status, 0) = 200 and not coalesce(v_timed, false) then
    ...
    if v_json is not null then
      ...
      -- Bara artist-matchande ORIGINAL. Inget → v_hit förblir null → hoppa vidare.
  ...
  -- Ingen original-träff: prova bredare sökterm (rå titel), därefter en annan låt.
